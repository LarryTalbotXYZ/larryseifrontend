...
interface ILeverageTokenFactory {
...
contract LeverageToken is ERC20Burnable, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Handle fee distribution based on factory burn configuration
     * @param totalFeeAmount Total fee amount to distribute
     */
    function _handleFeeDistribution(uint256 totalFeeAmount) private {
        bool shouldBurn = ILeverageTokenFactory(factory).isTokenBurnEnabled(address(backingToken));
        
        if (shouldBurn) {
            // Original logic: 80% to FEE_ADDRESS, 20% burned
            uint256 feeAddressAmount = (totalFeeAmount * 80) / 100;
            uint256 burnAmount = (totalFeeAmount * 20) / 100;
            ...
            // New logic: 100% to FEE_ADDRESS (no burning)
            backingToken.safeTransfer(FEE_ADDRESS, totalFeeAmount);
        }
    }

    /**
     * @notice Handle leverage fee distribution based on factory burn configuration
     * @param totalFeeAmount Total fee amount to distribute
     */
    function _handleLeverageFeeDistribution(uint256 totalFeeAmount) private {
        bool shouldBurn = ILeverageTokenFactory(factory).isTokenBurnEnabled(address(backingToken));
        
        if (shouldBurn) {
            // Original logic: 30% to FEE_ADDRESS, 10% burned
            uint256 feeAddressAmount = (totalFeeAmount * 3) / 10;
            uint256 burnAmount = (totalFeeAmount * 1) / 10;
            
            backingToken.safeTransfer(FEE_ADDRESS, feeAddressAmount);
            backingToken.safeTransfer(0x000000000000000000000000000000000000dEaD, burnAmount);
        } else {
            // New logic: 40% to FEE_ADDRESS (no burning, equivalent total)
            uint256 feeAddressAmount = (totalFeeAmount * 4) / 10;
            backingToken.safeTransfer(FEE_ADDRESS, feeAddressAmount);
        }
    }
    ...
    function buy(address receiver, uint256 amount) external nonReentrant {
        ...
        // Team fee: 1% total -> distributed based on burn configuration
        uint256 totalFeeAmount = amount / FEES_BUY;
        require(totalFeeAmount > MIN, "must trade over min");
        _handleFeeDistribution(totalFeeAmount);
    ...
    function leverage(uint256 backingAmount, uint256 numberOfDays) public nonReentrant {
        ...
        require(backingFee > MIN, "Fees must be higher than min.");
        _handleLeverageFeeDistribution(backingFee);
    ...
    function flashClosePosition() public nonReentrant {
        ...
        require(fee > MIN, "Fees must be higher than min.");
        _handleLeverageFeeDistribution(fee);
        subLoansByDate(borrowed, collateral, Loans[msg.sender].endDate);
    ...
    function extendLoan(
        ...
        require(loanFee > MIN, "Fees must be higher than min.");
        _handleLeverageFeeDistribution(loanFee);
