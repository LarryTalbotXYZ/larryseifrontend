...
contract LarryTalbotXYZ is ERC20Burnable, Ownable2Step, ReentrancyGuard {
    ...
    // Directly mint 100 tokens (with 18 decimals)
    uint256 teamMint = 100 * 10**18; 
   ...
   function mint(address to, uint256 value) private {
    ...
    function addToWhitelist(address _address) external onlyOwner {
        require(_address != address(0x0), "Cannot whitelist zero address");
    ...
    function buy(address receiver) external payable nonReentrant {
        ...
        if (whitelist[msg.sender]) {
            // No fees for whitelisted addresses
            mint(receiver, larry);
        } else {
            // Apply fees for regular addresses
    ...
    function safetyCheck(uint256 ETH) private {
        ...
        require(lastPrice <= newPrice + 10000, "The price of larry cannot decrease by more than 10000 wei");
        lastPrice = newPrice;
